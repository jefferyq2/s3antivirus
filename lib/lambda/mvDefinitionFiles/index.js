...
async function lambdaHandleEvent (event, context) {
  ...
  utils.ensureExists(constants.EFS_AVDEFINITION_DIR, (err) => {
  ...
  const filelist = await s3.listObjects(bucketParams, function (err, data) {
    if (err) {
      console.log('Error', err);
    } else {
      console.log('Successfully list all S3 files.');
      return data;
    }
  }).promise();
  ...
  for (let i = 0; i < filelist.Contents.length; i++) {
    ...
    if (key.substr(0,constants.PATH_TO_AV_DEFINITIONS.length) === constants.PATH_TO_AV_DEFINITIONS) {
      prs.push(downloadFileFromS3(filelist.Contents[i].Key, constants.CLAMAV_BUCKET_NAME));
    };
