...
function updateAVDefinitonsWithFreshclam () {
  try {
    console.log('FC path' + constants.PATH_TO_FRESHCLAM);
    console.log('Datadir:', constants.EFS_AVDEFINITION_DIR);
    // execSync(`${constants.PATH_TO_FRESHCLAM} --help`);

    const executionResult = execSync(`${constants.PATH_TO_FRESHCLAM} --no-dns --config-file=${constants.FRESHCLAM_CONFIG} --datadir=${constants.EFS_AVDEFINITION_DIR}`);
...
async function downloadAVDefinitions () {
  const downloadPromises = constants.CLAMAV_DEFINITIONS_FILES.map((filenameToDownload) => {
    return new Promise((resolve, reject) => {
      ...
      const s3ReadStream = S3.getObject(options).createReadStream().on('end', function () {
  ...
  const uploadPromises = constants.CLAMAV_DEFINITIONS_FILES.map((filenameToUpload) => {
    return new Promise((resolve, reject) => {
      ...
      const options = {
        ...
        Body:  fs.createReadStream(path.join(constants.EFS_AVDEFINITION_DIR, filenameToUpload))
      };
...
function scanLocalFile (pathToFile) {
  try {
    execSync(`${constants.PATH_TO_CLAMAV} -v -a --stdout -d /tmp/ '/mnt/lambda/tmp/${pathToFile}'`);
