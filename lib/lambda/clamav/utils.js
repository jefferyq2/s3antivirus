 ...
 * @param TagSet an array of Tags [{Key: string, Value: any}]
 * @return {{TagSet: *[]}} TagSet ready to be attached to an S3 file.
 */
function generateTagSet (virusScanStatus, TagSet = []) {
  const newTagset = [
    {
  ...
  return { TagSet: newTagset.concat(TagSet) };
}
...
function extractKeyFromS3Event (s3Event) {
  const key = s3Event.detail.requestParameters.key;
...
function extractBucketFromS3Event (s3Event) {
  const bucketName = s3Event.detail.requestParameters.bucketName;
