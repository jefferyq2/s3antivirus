...
/**
 * Checks if the file has the virusScanSkip tag
 * @param {object} tagging response object of a GetObjectTagging
 * @return {boolean} True if the tag exists and has the value 'yes'
 */
function virusScanSkip (tagging) {
  if ('TagSet' in tagging) {
    const skipTagSet = tagging.TagSet.filter(item => item.Key === 'virusScanSkip');
    if (skipTagSet.length > 0) {
      if (skipTagSet[0].Value === 'yes') {
        utils.generateSystemMessage('Skipping AV scan because tagging virusScanSkip found on the file.');
        return true;
      }
    }
  };
...
async function lambdaHandleEvent (event, context) {
  ...
  // Get object tags
  const tagging = await s3.getObjectTagging({ Bucket: s3ObjectBucket, Key: s3ObjectKey }).promise();

  // You need to verify that you are not getting too large a file
  // currently lambdas max out at 500MB storage.
  if ((constants.MAX_FILE_SIZE !== '0' && await isS3FileTooBig(s3ObjectKey, s3ObjectBucket)) || virusScanSkip(tagging)) {
  ...
  const taggingParams = {
    Bucket: s3ObjectBucket,
    Key: s3ObjectKey,
    Tagging: utils.generateTagSet(virusScanStatus, ('TagSet' in tagging ? tagging.TagSet : []))
  };
